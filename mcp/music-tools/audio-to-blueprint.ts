...
function usage(): never {
  ...
  console.error("Usage: npm run audio:blueprint -- <audio_path>");
  process.exit(2);
}
...
function sanitizeBlueprint(input: unknown, options: { transcript?: string } = {}): Blueprint {
  ...
  const id = typeof src.id === "string" && src.id.trim() ? src.id.trim() : `audio-${randomUUID()}`;
...
async function transcriptToBlueprint(client: OpenAI, transcript: string): Promise<Blueprint> {
  const voiceId = process.env.ELEVENLABS_DEFAULT_VOICE_ID ?? "21m00T1W";
  const modelId = process.env.ELEVENLABS_MODEL_ID ?? "eleven_multilingual_v2";
  const blueprintId = `audio-${randomUUID()}`;

  const prompt = [
    "You are generating a Song Blueprint JSON that MUST conform to this schema:",
    "- required: id, style, tempo_bpm (40-220), key (A-G with optional #/b and optional m), sections (>=1), lyrics (non-empty), voice.voice_id (non-empty)",
    "- optional: time_signature (default 4/4), section.energy (0-1), section.prompt, voice.model_id, voice.speaker_boost, metadata (object)",
    "- additionalProperties are NOT allowed at top-level, sections, or voice.",
    "- sections items MUST ONLY contain: name, bars, energy, prompt (no other keys, and do not put lyrics inside sections).",
    "",
    "Given this user transcript (spoken intent / humming description), produce a plausible pop song blueprint.",
    "Keep it safe and avoid using living artist names.",
    "",
    `Transcript: """${transcript}"""`,
    "",
    `Return ONLY valid JSON for the blueprint. Use id "${blueprintId}". Use voice.voice_id "${voiceId}". Use voice.model_id "${modelId}".`
  ].join("\n");
...
async function main() {
  ...
  // eslint-disable-next-line no-console
  console.log(JSON.stringify({ transcript, blueprint }, null, 2));
}
